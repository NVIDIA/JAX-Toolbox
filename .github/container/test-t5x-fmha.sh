...
args=$(getopt -o a:b:d:e:o:s:h --long additional-args:,batch-size:,dtype:,epochs:,help,multiprocess,output:,steps-per-epoch: -- "$@")
if [[ $? -ne 0 ]]; then
    exit 1
fi
...
## Check if HLO was dumped or not
if [ ! -d ${HLO_DIR} ]; then
	echo "E: ${HLO_DIR} was not created successfully!"
	exit 1
fi
