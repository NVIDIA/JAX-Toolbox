...
tests_name =  os.environ.get("TESTS_NAME", 'T5X')
baselines_dir = os.path.join(test_dir, os.environ.get("BASELINES_DIR", 'T5X_MGMN/upstream'))
...
def test_loss(baseline_filename):
    ...
    with open(baseline_filepath, "r") as baseline_file:
        ...
        assert_allclose(list(loss_actual.values()), list(loss_expected.values()),
                        rtol=LOSS_RTOL[tests_name][test_config],
...
def test_step_time(baseline_filename):
    ...
    with open(baseline_filepath, "r") as baseline_file:
        ...
        assert step_time_avg_actual > step_time_avg_expected * \
            STEP_TIME_MULT[tests_name][test_config], f"Step time values: {step_time_values} (Avg: {step_time_avg_actual}), Expected avg: {step_time_avg_expected}"
