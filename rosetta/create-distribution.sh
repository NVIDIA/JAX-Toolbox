...
A patchlist will be generated by this script and placed under $SCRIPT_DIR/patches/<library name> with all entries
replaced with local patches.
...
GEN_PATCH_DIR=${GEN_PATCH_DIR:-$SCRIPT_DIR/patches/$(basename $(git -C $INSTALLED_DIR remote get-url origin) .git)}
GEN_PATCH_LIST=$GEN_PATCH_DIR/$(basename $PATCH_LIST).gen
if [[ -e $GEN_PATCH_LIST ]]; then
  echo "[WARNING]: $GEN_PATCH_LIST exists and will be overwritten"
  rm -f $GEN_PATCH_LIST
fi
...
EOF
  ...
  echo "file://${patch_path#$SCRIPT_DIR/}" >> $GEN_PATCH_LIST
}
apply-patches() {
  from=$1
  to=$2
  ...
  # Apply the patch
  am+record $GEN_PATCH_DIR/$patch_fname
}
...
for line in $(cat ${PATCH_LIST}); do
  ...
  if [[ "${git_ref}" =~ ^file:// ]]; then
    ...
    if [[ -z "${EXTRA_DIR+x}" ]] || [[ ! -d ${EXTRA_DIR} ]]; then
      echo "[WARNING]: EXTRA_DIR=${EXTRA_DIR} does not exist so cannot apply patch=${git_ref}"
      continue
    fi
